              ...
              <div key={paper.id} className={`p-3 rounded-lg border transition-colors ${isSelected ? 'border-blue-600/50 bg-blue-600/10' : 'border-border bg-secondary/50 hover:border-accent'}`}>
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-medium text-sm text-foreground mb-1">{paper.title}</h3>
                    <div className="flex items-center gap-3 text-xs text-muted-foreground mb-2">
                      <span className="flex items-center gap-1"><Users size={12} /><span>{paper.authors.join(', ')}</span></span>
                      <span className="flex items-center gap-1"><Calendar size={12} /><span>{paper.year}</span></span>
                    </div>
                    <p className="text-xs text-muted-foreground/80 line-clamp-2 mb-2">{paper.abstract}</p>
                    <div className="flex items-center justify-between">
                      <div className="flex flex-wrap gap-1">
                        {paper.keywords.map(kw => <span key={kw} className="px-1.5 py-0.5 bg-accent/20 text-accent/80 text-xs rounded">{kw}</span>)}
                      </div>
